...
function addRoutes(host, app, options) {
  var uri = host + options.callback_path
    , api_base = options.api_base || 'https://api.familysearch.org/identity/v2/'
    , oa = createOAuth(api_base, options.key, null, uri)
  ...
  app.get(options.callback_path, hostChecker, callback.bind(null, oa))
